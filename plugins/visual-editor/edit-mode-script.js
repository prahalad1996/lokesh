...
function showPopup(targetElement, editId, currentText) {
  ...
  if (parentOrigin) {
    window.parent.postMessage({ type: 'editEnter' }, parentOrigin);
  }
}
...
function handleGlobalEvent(event) {
  if (!document.getElementById('root')?.getAttribute('data-edit-mode-enabled')) return;
  if (event.target.closest('#inline-editor-popup')) return;
  ...
  if (editableElement) {
    ...
    if (event.type === 'click') {
      if (currentEditingInfo) hidePopup();
...
async function handlePopupSave() {
  ...
  try {
    const response = await fetch(PLUGIN_APPLY_EDIT_API_URL, {
      ...
      body: JSON.stringify({ editId, newFullText: newText }),
    });
...
function handlePopupCancel() {
  const parentOrigin = getParentOrigin();
  if (parentOrigin) {
    window.parent.postMessage({ type: 'editCancel' }, parentOrigin);
  }
  hidePopup();
}
...
function showDisabledTooltip(targetElement) {
  ...
  if (newLeft < window.scrollX) newLeft = window.scrollX + 5;
  if (newLeft + tooltipWidth > window.innerWidth + window.scrollX) newLeft = window.innerWidth + window.scrollX - tooltipWidth - 5;
  if (newTop + tooltipHeight > window.innerHeight + window.scrollY) newTop = rect.top + window.scrollY - tooltipHeight - 5;
  if (newTop < window.scrollY) newTop = window.scrollY + 5;
